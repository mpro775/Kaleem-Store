          ...
          <Box sx={{ display: 'grid', gap: 3 }}>
            ...
            <Paper elevation={0} sx={{ p: { xs: 3, md: 4 }, borderRadius: 4, border: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                ...
                <Typography variant="h6" fontWeight={800}>فلاتر المنتج</Typography>
              </Box>
              <Divider sx={{ mb: 3 }} />

              {!selectedProduct ? (
                <Alert severity="info">احفظ المنتج أولاً لتثبيت إسناد الفلاتر.</Alert>
              ) : filters.length === 0 ? (
                <Alert severity="info">لا توجد فلاتر نشطة حالياً.</Alert>
              ) : (
                <Stack spacing={2}>
                  {filters.map((filter) => (
