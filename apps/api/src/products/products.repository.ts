...
export class ProductsRepository {
  ...
  async findById(storeId: string, productId: string): Promise<ProductRecord | null> {
    const result = await this.databaseService.db.query<ProductRecord>(
      `
        SELECT id, store_id, category_id, title, slug, description, status
        FROM products
        WHERE store_id = $1
          AND id = $2
        LIMIT 1
  ...
  async create(input: {
    ...
    const result = await this.databaseService.db.query<ProductRecord>(
      `
        INSERT INTO products (id, store_id, category_id, title, slug, description, status)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, store_id, category_id, title, slug, description, status
      `,
      [
        input.id,
        input.storeId,
  ...
  async updateVariantAttributes(input: {
    ...
    const result = await this.databaseService.db.query<ProductVariantRecord>(
      `
        ...
        WHERE store_id = $1
          AND id = $2
        RETURNING id, product_id, store_id, title, sku, barcode, price, compare_at_price, stock_quantity, low_stock_threshold, attributes, is_default
      `,
  ...
  async listVariants(storeId: string, productId: string): Promise<ProductVariantRecord[]> {
    const result = await this.databaseService.db.query<ProductVariantRecord>(
      `
        SELECT id, product_id, store_id, title, sku, barcode, price, compare_at_price, stock_quantity, low_stock_threshold, attributes, is_default
  ...
  private buildListQuery(input: {
    ...
    if (input.q) {
      conditions.push(
        `(p.title ILIKE '%' || $${nextParam} || '%' OR p.slug ILIKE '%' || $${nextParam} || '%')`,
      );
      values.push(input.q);
      nextParam += 1;
    }
